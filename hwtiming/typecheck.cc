...
void Module::typecheck(ostream&out, TypeEnv& env, map<perm_string,Module*> modules, char* depfun) const
{
	if (debug_typecheck) {
...
// output the encoding of a lattice with four labels with the
// following structure by default:
//                   HIGH
//             D1            D2
//                   LOW
//
// Programmers can define their own lattice structure in the file "lattice",
// using the [-l lattice_file] option to SecVerilog
...
// Predefined type-level functions.  
// Programmers can define their own function in the file "depfun",
 ...
 */
void typecheck(list<perm_string>roots, map<perm_string,Module*> modules, char* lattice_file_name, char* depfun_file_name)
{
      ...
      for (list<perm_string>::const_iterator root = roots.begin()
	    ...
	    // type environment \G and assumptions \A for type
	    // checking typing rules have the form \G, \A |- C, where
	    // \G is a map from Verilog vars to types, and \A is a
	    // conjunction of predicates on PWire
