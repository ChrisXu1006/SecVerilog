...
class ArrayIndexType : public SecType {

	public:
	  ArrayIndexType(perm_string name, perm_string array, PExpr* index);
	  ~ArrayIndexType();
	  ArrayIndexType& operator= (const ArrayIndexType&);
	  void dump(ostream&o) {
	  	o << "(" << name_ << " " << array_ << " " << index_ << ")";
	  }
	  bool equals(SecType* st);

	public:
	// Manipulate the types
	  void set_type(const perm_string name, perm_string array, PExpr* index);
	  perm_string get_name()  const;
	  perm_string get_array() const;
	  PExpr*	  get_index() const;
